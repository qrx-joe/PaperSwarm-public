...
本仓库是公开 demo 子集，包含工作流定义、角色提示词、schema/rubric 资源、静态 replay viewer、脱敏缓存示例、三个公开案例、冷启动 vs warm 启动召回实验，以及说明审计边界的证据文档。仓库不包含私有凭据、报名材料、行程记录、计划草稿、OAuth 实验文件或完整私有工作目录。
...
如果评委想先验证项目介绍中的关键主张，建议按这个顺序看：

- `docs/evidence/non-consensus-evidence.md`：医学 RCT 中统计审稿人与魔鬼代言人独立发现 CI/p 值不自洽，伦理审稿人补足临床合规视角。
- `docs/evidence/schema-gap-report.md`：说明哪些审计字段已经能从缓存 run 中恢复，哪些仍然需要未来 STEP 原生产出，避免把 demo 包装成未完成的能力。
- `experiments/cold-vs-warm-recall/comparison.md`：说明 EvoMap 召回记忆进入评审 prompt 后，对 warm 启动评审的影响。

这几份文档的价值是把“多角色蜂群”“EvoMap 结合”“可审计链路”从口号落到可打开的文件证据上。
...
|-- docs/evidence/              # 公开证据链和审计边界说明
|-- replay/                     # 静态回放页面
|-- scripts/                    # 离线检查和 Flowtrace 启动脚本
...
如果本机已经安装 Flowtrace CLI，可以用当前公开仓库里的脚本启动 trace DAG 查看器：
...
注意：Flowtrace 的 `--scope` 参数不是指 `trace.json` 所在目录，而是指“包含 trace 项目目录的父目录”。所以脚本会自动使用当前仓库的父目录作为 scope，让 `paperswarm-public/trace.json` 被正确发现。
...
另外，`case-studies/cs-ml-benchmark/` 展示计算机实验论文为什么需要复现审稿人；`case-studies/education-quasi-experiment/` 展示教育准实验案例中的真实 Worker/Verifier 修订验收闭环。
